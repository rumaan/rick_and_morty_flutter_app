...
void main() {
  test("Parses Characters from the API", () {
    ...
    expect(parseCharacter(characterJson).id, 1);
    expect(parseCharacter(characterJson).name, "Rick Sanchez");
  });
  ...
  test("Parses Episodes from the API", () {
    ...
    expect(parseEpisode(episodeJson).id, 1);
    expect(parseEpisode(episodeJson).name, "Pilot");

    expect(parseEpisode(episodeJson).characters[0],
        "https://rickandmortyapi.com/api/character/1");
  });
  ...
  test("Parses Location from the API", () {
    ...
    expect(parseLocation(locationJson).id, 1);
    expect(parseLocation(locationJson).type, "Planet");
    expect(parseLocation(locationJson).residents[0],
